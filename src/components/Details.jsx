        ...
        fetch(`http://localhost:5000/addmovies/${id}`, {
          ...
          .then((data) => {
            ...
            if (data.deletedCount > 0) {
              const remaining = movie.filter((mov) => mov._id !== id);
              setMovie(remaining);
              Swal.fire({
                title: "Deleted!",
          ...
          <div className="mb-10 bg-[#232323] w-fit rounded-lg p-6 gap-4">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="bg-[#DD003F] p-2 rounded-lg"
              >
                <FaTrash className="text-white" />
              </button>
              <button
                onClick={addToFav}
                className="font-medium border-[2px] border-[#DD003F] text-[#DD003F] px-3 py-2 rounded-lg hover:bg-[#DD003F] hover:text-[#1b1b1b] transition duration-500"
              >
                Add To Favorites
              </button>
            </div>
