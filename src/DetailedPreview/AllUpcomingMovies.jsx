    ...
    const fetchMovies = async () => {
      try {
        const res = await fetch(
          `https://api.themoviedb.org/3/movie/upcoming?language=en-US&page=${currentPage}&api_key=${API_KEY}`
        );
        const data = await res.json();
        setAllMovies(data.results || []);
        setTotalPages(data.total_pages > 100 ? 100 : data.total_pages); // TMDb caps at 500, we limit to 100
      } catch (error) {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-6 mt-6">
        {allMovies.map((movie) => (
          <div key={movie.id} className="bg-[#232323] p-2 rounded-lg group w-fit">
            <img
              className="lg:w-52 w-40 mx-auto rounded-lg group-hover:brightness-75 transition duration-300"
