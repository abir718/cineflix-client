...
function AllTopRatedTv() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      ...
      setAllTopRated(combined);
    };
    fetchMovies();
  }, [API_KEY]);

  const showsPerPage = 36;
  const totalPages = Math.ceil(allTopRated.length / showsPerPage);
  const startIndex = (currentPage - 1) * showsPerPage;
  const paginatedTv = allTopRated.slice(startIndex, startIndex + showsPerPage);
      ...
      <div className='flex items-center justify-center'>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-y-10 gap-x-6 mt-6">
          {paginatedTv.map((item) => (
            <div key={item.id} className="bg-[#232323] p-3 w-fit rounded-lg group">
              <img
